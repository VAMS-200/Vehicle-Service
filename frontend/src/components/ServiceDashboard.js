...
const handleLogout = () => {
  ...
  const fetchPendingRequests = async () => {
    try {
      const res = await axios.get("http://localhost:5001/request/pending");
...
const fetchActiveRequest = async () => {
  try {
    const res = await axios.get(
      `http://localhost:5001/request/active/${user._id}`
    );
    ...
    async (pos) => {
      try {
        const res = await axios.put(
          `http://localhost:5001/request/accept/${requestId}`,
          {
        ...
        const fullReq = await axios.get(
          `http://localhost:5001/request/${requestId}`
        );
...
const handleConfirmCompletion = async (requestId) => {
  try {
    await axios.put(`http://localhost:5001/request/confirm-completion/${requestId}`);
  ...
  const startLocationUpdates = (requestId) => {
    setInterval(() => {
      navigator.geolocation.getCurrentPosition(async (pos) => {
        try {
          await axios.put(
            `http://localhost:5001/request/update-location/${requestId}`,
            {
              serviceManLat: pos.coords.latitude,
              serviceManLng: pos.coords.longitude,
            }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await axios.get(
          `http://localhost:5001/request/${activeRequest._id}`
        );
        setActiveRequest(res.data);
